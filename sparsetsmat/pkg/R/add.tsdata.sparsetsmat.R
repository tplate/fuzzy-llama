...
add.tsdata.sparsetsmat <- function(x,
    ...
    if (!identical(class(x.df[[3]]), class(new.df[[3]])) && is.logical(new.df[[3]]) && all(is.na(new.df[[3]])))
        storage.mode(new.df[[3]]) <- storage.mode(x.df[[3]])
    if (!identical(class(x.df[[3]]), class(new.df[[3]])) && is.numeric(new.df[[3]]) && is.numeric(x.df[[3]]))
        storage.mode(new.df[[3]]) <- storage.mode(x.df[[3]])
